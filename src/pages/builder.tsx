...
export const query = graphql`
  query {
    allBuilderModels {
      # (optional) example custom "header" component model, if you have one - target: { urlPath: "/header" },
      header(limit: 1, options: { cachebust: true }) {
        content
      }
      # Manually grab the page content matching name="builderform"
      # For Gatsby content, we want to make sure to always get fresh (cachebusted) content
      page(limit: 1, target: { name: "builderform" }, options: { cachebust: true }) {
        content
      }
    }
  }
